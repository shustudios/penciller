...
export default {
  ...
  data: function() {
    return {
      ...
      maskedValue: this.value || { from:'', to:'' },
    }
  },
  watch: {
    value: {
  ...
  computed: {
    localValue: {
      get: function () {
        ...
        if (!this.isUndefined(this.value) && this.value.hasOwnProperty('from')) {
          output.from = this.value.from
        }

        if (!this.isUndefined(this.value) && this.value.hasOwnProperty('to')) {
          output.to = this.value.to
        }
